...
def get_piece(x, y, board):
    ...
    assert type(x) == int, "int only"
    assert type(y) == int, "int only"
...
def swipe_left(board):
    """
    Swipe function moves in left direction until nothing can be moved any further.
    Arg board: board - (Board is the name of function's parameter)
    """
    
    #Checks if an action has been taken
    action_taken = False

    #Measures the board's dimensions
    N = len(board)

    #loops through each x and y coordinate
    for y in range(N):
        for x in range(N):
            #gets exact position on board
            piece_at_xy = get_piece(x, y, board)
            left_adjacent = get_piece(x-1, y, board)

            #if the piece is at specific location, continue the game
            if piece_at_xy == '*':
                continue

            #if not out of bounds left side, continue
            if left_adjacent == None:
                continue

            #lets user make a move
            action_taken = move(x, y, "left", board) or action_taken


    #if user moves, end the move and move on
    if action_taken:
        end_move(board)
